...
Vérifier l'émission du certificat wildcard (peut prendre 1-2 min — propagation
du TXT DNS) :

```bash
docker exec traefik sh -c 'wc -c < /etc/traefik/acme/acme.json'   # > 3000 = cert obtenu
docker logs traefik 2>&1 | grep -i "Loading ACME certificates"
```
...
curl -sI https://demo.deljdlx.fr/ | grep -i location                 # HTTPS OK (cert Let's Encrypt)
```
...
7. **Staging avant prod.** Let's Encrypt a des quotas stricts. Tester d'abord
   avec l'autorité *staging* (cf. commentaire dans `traefik.yml`), puis basculer
   en prod. En changeant d'autorité, **vider `acme/acme.json`** (le compte
   staging n'est pas valide en prod) :
   ```bash
   docker compose down && : > acme/acme.json && docker compose \
